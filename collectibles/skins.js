...
fetch("../databases/shop_items.json")
    ...
    .then((data) => {
        var skins = data["skins"];
        var skindiv = document.getElementById("magicalSkins");
        for (let i in skins) {
            if (skins[i]["era"] === "magical") {
                skindiv.innerHTML =
                    skindiv.innerHTML +
                    ("<p>Name:<span class = 'gold'> " + skins[i]["skin"] + " " + skins[i]["item"] + "</span><br />") +
                    ("Cost: " + skins[i]["price"] + " " + skins[i]["cost"] + "</p>");
...
fetch("../databases/shop_items.json")
    ...
    .then((data) => {
        var skins = data["skins"];
        var skindiv = document.getElementById("egyptianSkins");
        for (let i in skins) {
            if (skins[i]["era"] === "egyptian") {
                skindiv.innerHTML =
                    skindiv.innerHTML +
                    ("<p>Name:<span class = 'gold'> " + skins[i]["skin"] + " " + skins[i]["item"] + "</span><br />") +
                    ("Cost: " + skins[i]["price"] + " " + skins[i]["cost"] + "</p>");
...
fetch("../databases/shop_items.json")
    ...
    .then((data) => {
        const old_event = data["christmas"];

        for (var i = 0; i < old_event.length; i++) {
            document.getElementById("christmasEvent").innerHTML =
                document.getElementById("christmasEvent").innerHTML +
...
fetch("../databases/shop_items.json")
    ...
    .then((data) => {
        const old_event = data["halloween"];

        for (var i = 0; i < old_event.length; i++) {
            document.getElementById("halloweenEvent").innerHTML =
                document.getElementById("halloweenEvent").innerHTML +
                ("<p> Name: <span class = 'gold'>" + old_event[i].skin + " " + old_event[i].item + "</span> <br>") +
                ("Era: <b>" + old_event[i].era + "</b><br>") +
                ("Price: " + old_event[i].price + " " + old_event[i].cost + "</p>");
...
fetch("../databases/shop_items.json")
    ...
    .then((data) => {
        const old_event = data["old_old_event"];
